...
| sample_step3.csv | ステップ3完了時点 | assign_filenames.py によりJ列記入・G/H列置換済み |
...
J列（ファイル名）に通し番号付きファイル名が書き込まれています。G列・H列のブラケット内のURLがファイル名に置換されています。

```
G列の例: [EVD001|Users are required to...]   ← URLがEVD001に置換
H列の例: [EVD001|ユーザーはシステムに...]     ← 同上
J列の例: EVD001                                ← ファイル名が記入
```
...
`assign_filenames.py` では、同一URLに対して同一のファイル名が割り当てられ、重複したPDF生成が防止されます。`url2pdf.py` では、J列の重複排除により同じURLからPDFが2回生成されることはありません。`pdf_annotate.py` では、同一PDF内の異なるテキスト箇所にそれぞれハイライトが付与され、項番ラベルは同一ページ内で重複排除されます。
...
※ step3 でファイル名がK列（11列目）になっていますが、これはCSVの列数に依存します。実際のExcel運用ではJ列（10列目）に書き込まれます。
